...
const OrderDetails = () => {
 ...
 <motion.div className="w-12 h-12 border-4 border-primary/40 border-t-primary rounded-full"animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }} />
 </div>
);
 }
 ...
 <div className="mb-8 flex items-center gap-3 text-[10px] font-bold tracking-[0.2em] uppercase text-primary/70 print:hidden">
 ...
 <div className="flex flex-col md:flex-row justify-between gap-8 mb-16 pb-12 border-b border-border/70">
 <div>
...
"{order.deliveryInstructions}"
 ...
 <div className="mb-16 pb-12 border-b border-border/70 print:hidden">
 ...
 <tr key={i} className="border-b border-border/70 last:border-0 group hover:bg-surface/30 transition-colors print:hover:bg-transparent">
 <td className="py-6">
 <div className="flex items-center gap-5">
 <div className="w-16 h-16 rounded-xl overflow-hidden bg-surface border border-border flex-shrink-0 print:hidden">
 <img src={item.productId?.image || premiumFallback} alt={item.productId?.name} className="w-full h-full object-cover"onError={(e) => { e.target.onerror = null; e.target.src = premiumFallback; }} />
 </div>
 ...
 <div className="flex flex-col md:flex-row justify-between items-end md:items-start gap-12 pt-12 border-t border-border/70">
