  ...
  return (
    <div className="min-h-screen flex" id="main-content">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-primary">
        ...
        <div className="absolute top-20 -left-10 w-40 h-40 bg-gold/5 rounded-full blur-[60px]" aria-hidden="true" />
        ...
        <div className="relative z-10 flex flex-col justify-between p-14 w-full">
          ...
          <div className="bg-white/6 border border-white/10 rounded-2xl p-6">
            <div className="flex gap-1 mb-3" aria-label="5 stars">
              {[1,2,3,4,5].map(s => <svg key={s} className="w-4 h-4 text-gold" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>)}
            </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16 bg-cream">
        ...
        >
          ...
          <h1 className="font-display text-3xl font-semibold text-primary mb-2">Sign In</h1>
          <p className="text-muted text-sm mb-8">Don't have an account?{' '}
          ...
          <div className="relative flex items-center gap-4 mb-6" role="separator" aria-label="Or sign in with email">
            ...
            <span className="text-xs text-muted font-medium tracking-wide">or continue with email</span>
            <div className="flex-1 h-px bg-border" aria-hidden="true" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate aria-label="Sign in form">
            {/* Email */}
            <div>
              <label htmlFor="login-email" className="block text-xs font-bold text-muted tracking-wide uppercase mb-1.5">Email Address</label>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label htmlFor="login-password" className="text-xs font-bold text-muted tracking-wide uppercase">Password</label>
                <a href="#" className="text-xs font-semibold text-gold hover:text-gold-dark transition-colors">Forgot password?</a>
              </div>
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted" aria-hidden="true">
          ...
          <p className="text-center mt-6 text-xs text-muted">
            New to BazarGhar?{' '}
            <Link to="/register" className="font-bold text-primary hover:text-gold transition-colors">Create an account →</Link>
          </p>
        </motion.div>
      </div>
    </div>
