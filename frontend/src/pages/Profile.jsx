...
const Profile = () => {
 ...
 <p className="text-[10px] font-bold text-primary/70 uppercase tracking-widest">{s.label}</p>
 </div>
))}
 </div>
 ...
 <p className="font-medium text-primary">{user?.phone || <span className="text-primary/60 italic">Not provided</span>}</p>
 </div>
 <div>
 <p className="label-editorial mb-1">Default Address</p>
 <p className="font-medium text-primary line-clamp-2">{user?.address || <span className="text-primary/60 italic">Not provided</span>}</p>
 </div>
 </div>
 </div>
 ...
 >
 <div className="bg-surface/30 px-6 lg:px-8 py-5 flex flex-wrap items-center justify-between gap-5 border-b border-border/70">
 ...
 <div className="flex items-center gap-4">
 <span className="text-[11px] font-mono font-bold tracking-widest text-primary/70">
 ...
 <img src={item.productId?.image || premiumFallback} alt={item.productId?.name} loading="lazy"className="w-full h-full object-cover"onError={(e) => { e.target.onerror = null; e.target.src = premiumFallback; }} />
 </div>
 ...
 <p className={`text-[9px] font-bold uppercase tracking-widest text-center ${done ? 'text-secondary' : 'text-primary/60'}`}>{s}</p>
 </div>
 ...
 <div className="mt-8 pt-6 border-t border-border/70 flex flex-col sm:flex-row gap-4 justify-between items-center">
 ...
 <input id="profile-email"type="email"value={user?.email || ''} disabled className="input-base bg-surface text-primary/70 cursor-not-allowed"/>
 <p className="text-[10px] text-primary/60 mt-1.5 font-medium">Email cannot be changed</p>
 </div>
 <div>
 ...
 >
 <svg className={`w-5 h-5 shrink-0 ${activeTab === tab.id ? 'text-surface-white' : 'text-primary/70'}`} fill="none"viewBox="0 0 24 24"strokeWidth={2} stroke="currentColor">
 ...
 {/* Logout Button */}
 <div className="mt-8 lg:mt-12 border-t border-border/70 pt-8 hidden lg:block">
