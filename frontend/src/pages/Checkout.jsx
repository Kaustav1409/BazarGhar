...
const inputClass = 'input-base w-full bg-surface-white focus:bg-white transition-colors duration-300';
            ...
            {['Shipping', 'Payment'].map((label, i) => {
              ...
              return (
                <React.Fragment key={label}>
                  <div className={`flex flex-col items-center gap-2 ${active ? 'text-primary' : done ? 'text-primary' : 'text-primary/30'} transition-colors`}>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                <motion.div key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }} className="space-y-10">
                  <div className="card-premium p-8 lg:p-12 !rounded-[2.5rem]">
              ...
              {step === 2 && (
                <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }} className="space-y-10">
                  ...
                  <div className="card-premium p-8 lg:p-10 !rounded-[2.5rem]">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="font-heading text-2xl font-bold text-primary">Delivery Details</h3>
                      <button onClick={() => setStep(1)} className="text-[11px] uppercase tracking-widest font-bold text-primary/50 hover:text-primary transition-colors">Edit</button>
                    </div>
                    ...
                    <div className="space-y-4">
                      {PAYMENT_METHODS.map((method) => {
                        ...
                        return (
                          ...
                          >
                            {active && <div className="absolute inset-0 bg-primary opacity-10 pointer-events-none" />}
                            
                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${active ? 'border-surface-white' : 'border-primary/20 group-hover:border-primary/50'}`}>
                            ...
                            <div className="flex flex-col flex-1">
                              ...
                              <p className={`text-[12px] font-medium mt-1 ${active ? 'text-surface-white/70' : 'text-primary/50'}`}>{method.desc}</p>
                            </div>

                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-sm shrink-0 ${active ? 'bg-surface-white/10 text-surface-white' : 'bg-surface text-primary/40'}`}>
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: "easeOut" }} className="w-full text-center py-20 lg:col-span-12">
                  ...
                  {order && (
                    <div className="card-premium p-8 !rounded-[2rem] max-w-md mx-auto text-left mb-12">
                      <div className="flex justify-between items-center mb-6 pb-6 border-b border-border/60">
                        <div>
                          <p className="text-[10px] font-bold text-primary/40 uppercase tracking-[0.2em] mb-2">Order Reference</p>
                          <p className="font-mono text-lg font-bold text-primary">#{order._id.slice(-8).toUpperCase()}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[10px] font-bold text-primary/40 uppercase tracking-[0.2em] mb-2">Payment</p>
                          <p className="text-[13px] font-bold text-primary uppercase">{order.paymentMethod}</p>
                        </div>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-primary/40 uppercase tracking-[0.2em] mb-2">Estimated Arrival</p>
                  ...
                  {cartItems.map((item) => (
                    <div key={item._id} className="flex gap-4 items-center">
                      ...
                      <div className="flex-1 min-w-0 py-1">
                        ...
                        <p className="text-[11px] text-primary/50 font-medium mt-1">Qty: {item.quantity}</p>
                      </div>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-surface-white border border-border flex items-center justify-center text-primary/40">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
