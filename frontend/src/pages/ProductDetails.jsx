...
const ProductDetails = () => {
 ...
 <svg className="w-10 h-10 text-primary/60"fill="none"viewBox="0 0 24 24"strokeWidth={1} stroke="currentColor"aria-hidden="true">
 ...
 // Stacked Gallery Images (Using product image for missing angles)
 const galleryImages = [
 product.image,
 product.image,
 product.image,
 ...
 {/* Breadcrumb */}
 <nav className="flex items-center gap-2 text-[11px] font-bold tracking-widest uppercase text-primary/70 mb-12"aria-label="Breadcrumb">
 ...
 className="absolute top-6 right-6 z-10 w-12 h-12 glass-dark rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-soft"
 aria-label="Wishlist"
 >
 ...
 <img src={img || premiumFallback} alt=""loading="lazy"decoding="async"className="w-full h-full object-cover"onError={(e) => { e.target.onerror = null; e.target.src = premiumFallback; }} />
 {idx === 0 && discountPct && (
 ...
 {originalPrice && (
 <p className="text-xl text-primary/60 line-through font-semibold leading-none">{formatPrice(originalPrice)}</p>
)}
 </div>
 <p className="text-[12px] text-primary/70 font-medium tracking-wide">Inclusive of all taxes · Free delivery on luxury orders</p>
 </div>
 ...
 {/* Specifications Grid */}
 <div className="grid grid-cols-2 gap-x-8 gap-y-6 pt-8 border-t border-border/70">
 {[
 ...
 <div key={i}>
 <p className="label-editorial mb-1 text-primary/60">{spec.label}</p>
 ...
 <div className="card-premium p-5 border-secondary/20 bg-secondary/5 flex flex-col gap-2">
 ...
 className="w-12 h-full flex items-center justify-center text-xl text-primary/70 hover:text-primary hover:bg-surface rounded-xl transition-all"
 disabled={quantity <= 1}
 >−</button>
 ...
 <svg className="w-5 h-5 text-primary/60 shrink-0"fill="none"viewBox="0 0 24 24"strokeWidth={1.5} stroke="currentColor">
 ...
 <span className="text-lg text-primary/70 font-medium">out of 5</span>
 </div>
 <StarRating rating={product.rating || 0} count={product.reviews || 128} />
 </div>
 ...
 <span className="text-primary/70 font-medium w-8 text-right">{pct}%</span>
 </div>
))}
 </div>
 ...
 <p className="text-[11px] text-primary/70 tracking-wide font-medium mt-0.5">{review.date}</p>
 </div>
 </div>
