...
const StarRating = ({ rating }) => (
  <div className="flex items-center gap-1 mt-1" aria-label={`${rating.toFixed(1)} out of 5 stars`}>
    {[1, 2, 3, 4, 5].map((star) => (
      <svg
        key={star}
        className={`w-2.5 h-2.5 ${star <= Math.round(rating) ? 'text-primary' : 'text-primary/15'}`}
        fill="currentColor"
        viewBox="0 0 20 20"
        aria-hidden="true"
      >
...
const ProductCard = ({ product }) => {
  ...
  return (
    <motion.article
      ...
      className="group flex flex-col h-full card-premium relative !rounded-[1.25rem]"
    ...
    >
      <Link to={`/product/${product._id}`} className="flex flex-col h-full relative z-10" aria-label={`View ${product.name}`}>
        ...
        <div className="relative w-full aspect-[4/5] bg-surface-secondary overflow-hidden">
          {/* Skeleton */}
          {!imgLoaded && !imgError && (
          ...
          {!imgError ? (
            <img
              ...
              loading="lazy"
              decoding="async"
              className={`w-full h-full object-cover transition-transform duration-[1.2s] ease-[0.25,1,0.25,1] group-hover:scale-105 ${
          ...
          <button
            ...
            className="absolute top-3 right-3 w-9 h-9 flex items-center justify-center glass-white rounded-full border border-surface-white/40 shadow-sm transition-all duration-300 hover:scale-110 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 z-20"
            ...
            <button
              ...
              className={`w-full py-3 text-[10px] font-semibold font-body tracking-[0.15em] uppercase rounded-xl transition-all duration-300 backdrop-blur-md border ${
                inCart
                  ...
                  : product.stock === 0
                  ? 'bg-surface-white/90 text-primary/40 border-border cursor-not-allowed'
          ...
          <div className="absolute top-3 left-3 flex flex-col items-start gap-1.5 z-20">
            {/* Category */}
            <span className="badge-neutral glass-white backdrop-blur-md">
              {product.category}
            </span>
        ...
        <div className="p-4 sm:p-5 flex flex-col flex-1 bg-surface-white justify-between">
          <div className="mb-3">
            <h3 className="font-heading text-[14px] font-semibold text-primary leading-snug line-clamp-2 group-hover:text-primary/70 transition-colors duration-300">
              {product.name}
            </h3>
          ...
          <div className="flex items-end justify-between mt-auto pt-4 border-t border-border/40">
            <div className="flex flex-col">
              {originalPrice && (
                <span className="text-[11px] text-primary/40 font-medium line-through mb-0.5">
