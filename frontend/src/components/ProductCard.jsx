...
const StarRating = ({ rating }) => (
  <div className="flex items-center gap-0.5" aria-label={`${rating.toFixed(1)} out of 5 stars`}>
    {[1, 2, 3, 4, 5].map((star) => (
      <svg
        key={star}
        className={`w-3 h-3 ${star <= Math.round(rating) ? 'text-gold' : 'text-border'}`}
        fill="currentColor"
        viewBox="0 0 20 20"
        aria-hidden="true"
      >
    ...
    <span className="ml-1.5 text-[11px] text-muted font-medium">{rating.toFixed(1)}</span>
  </div>
);
...
const ProductCard = ({ product }) => {
  ...
  return (
    <motion.article
      ...
      className="group bg-white border border-border/60 rounded-3xl overflow-hidden hover:shadow-card-hover hover:border-border transition-all duration-400 cursor-pointer flex flex-col h-full"
      ...
      <Link to={`/product/${product._id}`} className="block flex-1 flex flex-col" aria-label={`View ${product.name}`}>
        ...
        <div className="relative overflow-hidden aspect-[4/5] bg-cream">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.85 }}
            className="absolute top-3 right-3 w-9 h-9 flex items-center justify-center bg-white/95 backdrop-blur-sm rounded-full shadow-soft transition-all duration-200 hover:scale-110 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
            aria-label={isWishlisted ? 'Remove from wishlist' : 'Add to wishlist'}
          >
            <motion.svg
              className={`w-4 h-4 transition-colors ${isWishlisted ? 'text-red-500' : 'text-ink/50'}`}
          ...
          <div className="absolute inset-x-0 bottom-0 px-3 pb-3 translate-y-full group-hover:translate-y-0 transition-transform duration-350 ease-out">
            <button
              ...
              className={`w-full py-3 text-xs font-bold tracking-wider uppercase rounded-2xl transition-all duration-300 ${
                inCart
                  ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 cursor-default'
                  : product.stock === 0
                  ? 'bg-white/80 text-muted cursor-not-allowed border border-border'
                  : 'bg-primary text-white hover:bg-ink shadow-lg hover:shadow-xl'
              }`}
          ...
          <div className="absolute top-3 left-3">
            <span className="px-2.5 py-1 bg-white/95 backdrop-blur-sm text-[10px] font-bold text-ink/70 tracking-wider uppercase rounded-lg shadow-sm">
              {product.category}
            </span>
          </div>
        ...
        <div className="p-6 flex-1 flex flex-col">
          <div className="flex-1">
            <h3 className="text-sm font-semibold text-primary leading-snug line-clamp-2 mb-2.5 group-hover:text-gold-dark transition-colors duration-300">
              {product.name}
            </h3>
          ...
          <div className="flex items-end justify-between mt-4 pt-4 border-t border-border/40">
            ...
            <div className="text-right">
              ...
              {product.stock === 0 && (
                <span className="text-[10px] font-semibold text-red-500 bg-red-50 border border-red-100 px-2.5 py-0.5 rounded-full">
                  Sold out
                </span>
              )}
            </div>
