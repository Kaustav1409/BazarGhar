...
const Navbar = () => {
 ...
 // We are fully in Luxury Dark Mode.
 
 const linkBase = `relative text-[11px] font-body uppercase tracking-[0.25em] font-medium transition-colors duration-500 pb-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary/50 rounded`;
 const linkIdle = 'text-primary-light/90 hover:text-primary-light';
 const linkActive = 'text-secondary';
 
 const iconIdle = 'text-primary/80 hover:text-secondary hover:bg-surface-secondary/50';

 return (
 <>
 <a
 href="#main-content"
 className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[100] focus:bg-surface focus:text-primary focus:px-4 focus:py-2 focus:rounded-lg focus:text-sm focus:font-medium"
 >
 ...
 className="fixed top-0 left-0 right-0 z-50 bg-brand text-primary-light overflow-hidden"
 >
 <div className="max-w-7xl mx-auto px-6 py-2.5 flex items-center justify-between gap-4">
 <div className="flex-1 text-center text-[10px] md:text-[11px] font-medium tracking-[0.25em] uppercase">
 <span className="text-secondary font-semibold">Premium Experience</span>
 <span className="text-[#F7F3EC] tracking-widest hidden sm:inline"> — Free shipping on all luxury orders above ₹1999. Use code </span>
 <span className="text-[#F7F3EC] tracking-widest sm:hidden"> — Use code </span>
 <span className="text-secondary font-bold tracking-[0.3em]">BAZAAR10</span>
 </div>
 <button
 onClick={() => setAnnouncementVisible(false)}
 className="flex-shrink-0 text-primary-light hover:text-secondary hover:bg-white/10 transition-colors p-1.5 rounded-full"
 ...
 announcementVisible ? 'top-[38px]' : 'top-0'
 } ${
 scrolled
 ? 'bg-surface/80 backdrop-blur-xl shadow-card border-b border-border/60 py-4'
 : 'bg-gradient-to-b from-surface/80 to-transparent py-6'
 }`}
 >
 ...
 >
 <span className={`font-brand text-3xl lg:text-4xl font-extrabold tracking-tight transition-colors duration-500 text-primary-light`}>
 BazarGhar
 </span>
 <span className={`text-[8px] lg:text-[9px] font-body font-medium tracking-[0.4em] uppercase mt-1.5 hidden sm:block text-primary-light/90`}>
 ...
 {/* Dropdown */}
 <div className="absolute right-0 top-full mt-2 w-56 card-premium py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 origin-top-right scale-95 group-hover:scale-100 z-50">
 ...
 <button onClick={openCart} className="relative p-2 text-primary-light"aria-label="Open cart">
 ...
 className="p-2 relative z-50 text-primary-light"
 aria-label={mobileOpen ? 'Close menu' : 'Open menu'}
 aria-expanded={mobileOpen}
 >
 <motion.div animate={mobileOpen ? 'open' : 'closed'} className="w-5 h-5 flex flex-col justify-center gap-[5px]">
 <motion.span
 variants={{ open: { rotate: 45, y: 7, scaleX: 1, backgroundColor: '#A38560' }, closed: { rotate: 0, y: 0, scaleX: 1, backgroundColor: '#FFFFFF' } }}
 className="w-5 h-[1.5px] block origin-center"
 transition={{ duration: 0.5, ease: [0.25, 1, 0.25, 1] }}
 />
 <motion.span
 variants={{ open: { opacity: 0, scaleX: 0 }, closed: { opacity: 1, scaleX: 1, backgroundColor: '#FFFFFF' } }}
 ...
 <span className="font-brand text-3xl font-extrabold text-primary">BazarGhar</span>
 ...
 <Link
 to={to}
 className="font-body text-4xl md:text-5xl font-medium text-primary/80 hover:text-secondary transition-colors py-6 block"
 ...
 {isAuthenticated ? (
 <>
 <Link to="/profile"className="flex items-center gap-4 text-lg font-medium text-primary/80 hover:text-secondary transition-colors"onClick={() => setMobileOpen(false)}>
 ...
 <Link to="/register"className="btn-outline !text-primary !border-primary/40 hover:!border-secondary hover:!text-secondary"onClick={() => setMobileOpen(false)}>Create Account</Link>
 </div>
)}
 </div>
 </motion.div>
